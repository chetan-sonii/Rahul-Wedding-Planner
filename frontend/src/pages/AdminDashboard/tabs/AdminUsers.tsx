...
// --- 1. Enhanced Types to match populated data ---
...
interface User {
    ...
    budget?: number;
    guestCount?: number;
    favorites?: ShortlistedVendor[]; // This receives the full objects now
}
...
const AdminUsers = () => {
    ...
    const fetchUsers = async () => {
        try {
            ...
            // The backend's .populate('favorites') ensures we get vendor details here
    ...
    // 3. Helper: Calculate Total Cost
    const calculateShortlistCost = (favorites: any[] = []) => {
        if (!favorites || !Array.isArray(favorites)) return 0;
        return favorites.reduce((acc, curr) => {
            // Check if curr is a populated object (has price) or just an ID
            return acc + (typeof curr === 'object' && curr.price ? curr.price : 0);
        }, 0);
    };
                                ...
                                {selectedUser.favorites && selectedUser.favorites.length > 0 ? (
                                    <div className="space-y-4">
                                        {selectedUser.favorites.map((fav: any) => {
                                            // Guard clause: If backend accidentally sends just IDs, skip or show placeholder
